...
func TestCreateRequest(t *testing.T) {
	request := CreateRequest("AddTaskRequest", "ed1605e17374bde6c68864d072c9f5c9", "nginx:1.14", "", "", "2")
	if AddTask_util(address, request).Status != "Success" {
		t.Fail()
	}
...
func TestAddtask_util(t *testing.T) {
	request := CreateRequest(1, "ed1605e17374bde6c68864d072c9f5c9", "nginx:1.14", "", "", "2")
	if AddTask_util(address, request).Status != "Success" {
		t.Fail()
	}
}
...
func TestCancelTask_util(t *testing.T) {
	...
	if CancelTask_util(address, request).Status != "Failure" {
		t.Fail()
	}
}
